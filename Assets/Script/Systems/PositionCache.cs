...
// PositionCache
// 
// 概要: Transform.positionへの頻繁なアクセスを最適化するキャッシュシステム
// 
// 制作者: 小さな座布団
// 
// 機能説明:
// Transform情報(位置、回転、方向ベクトル)をキャッシュし、頻繁なネイティブコール削減によるパフォーマンス向上を実現。
// 毎フレーム前フレームとの差分から移動速度・移動量を自動計算。
// 他のPositionCacheとの相対位置計算や距離判定などのユーティリティ機能を提供。
// 
// 入力元クラス: Transform (Unity標準)
// 出力先クラス: 各種Systemクラス、CameraSystem等
// 
// その他:
// Update()で毎フレーム自動更新されるため、使用側は公開プロパティを参照するだけで最新情報を取得可能
